        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_APSReceiptSync;
                    ...
                    logger.message = "APS receipt sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_receipt");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT varTrxNo , charRef , dtBankIndate , decTotalAmt , (decTotalAmt - decOpenCredit) as decOffSetAmt, col.blnIsDelete from vwActiveSal_CollectionTbl col inner join Sal_CustomerTbl cust on col.intCustID = cust.intCustID";
                        ...
                        {
                            throw new Exception("APS Receipt sync requires backend rules");
                        }
                    ...
                    logger.message = "APS receipt sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
