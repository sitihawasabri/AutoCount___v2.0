...
{
    public class JobAPSWarehouseSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_APSWarehouseSync;                                 /*check again */
                    ...
                    logger.message = "APS warehouse sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_warehouse");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT * FROM Inv_WarehouseTbl";
                    ...
                    slog.action_identifier = Constants.Action_APSWarehouseSync;
                    ...
                    LocalDB.InsertSyncLog(slog);
                    logger.message = "APS warehouse sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
