...
namespace EasySales.Job
{
    ...
    class JobAPSCreditNoteKOSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_APSCreditNoteKOSync;
                    ...
                    logger.message = "APS Aging KO sync is running";
                    logger.Broadcast();

                    //query = "INSERT INTO cms_customer_ageing_ko(doc_code,doc_ko_ref,doc_ko_type,doc_amount,from_doc) VALUES ";
                    //updateQuery = " ON DUPLICATE KEY UPDATE ko_doc=VALUES(doc_amount);";
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_customer_ageing_ko_cn");

                        Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_customer_ageing_ko_cn");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "select cn.varTrxNo as cn_code, cnoff.decOffSetAmt as ko_amount, inv.varTrxNo as invoice_code, cnoff.charType from Sal_CNTbl as cn left join Sal_CNOffSetTbl cnoff on cnoff.intCNNo = cn.intCNNo left join Sal_InvoiceTbl inv on inv.intIVNo = cnoff.intRefNo where cnoff.charType = 'INV' ";
                                    ...
                                    if (cms_updated_time.Count > 0)
                                    {
                                        ...
                                        string where = string.Format("  AND cn.dtModifyDate  > '{0}'", updated_at);
                                        query += where;
                                    }
                        ...
                        {
                            throw new Exception("APS Aging KO CN sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<APSRule>((database, idx) =>
                        {
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    logger.message = string.Format("{0} Aging KO CN records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                            ...
                            if (cms_updated_time.Count > 0)
                            {
                                mysql.Insert("UPDATE cms_update_time SET updated_at = NOW() WHERE table_name = 'cms_customer_ageing_ko_cn'");
                            }
                            else
                            {
                                mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_customer_ageing_ko_cn', NOW())");
                            }
                    ...
                    logger.message = "APS Aging KO CN sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
