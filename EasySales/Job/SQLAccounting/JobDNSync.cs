        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_details = Constants.Tbl_cms_debitnote + Constants.Is_Starting;    /*check again */
                ...
                CHECKAGAIN:
                    ...
                    dynamic lDataSet;
                    string lSQL, DOCNO, CODE, DOCDATE, DOCAMT, PAYMENTAMT, CANCELLED, AGENT, CURRENCYRATE;
                    ...
                    Dictionary<string, string> cms_updated_time = _mysql.GetUpdatedTime1YearInterval("cms_debitnote"); 
                    ...
                    string getActiveDN = "SELECT dn_code FROM cms_debitnote WHERE cancelled = 'F'";
                    if (cms_updated_time.Count > 0)
                    {
                    ...
                    //lSQL = "SELECT DOCNO,CODE,DOCDATE,DOCAMT,PAYMENTAMT,CANCELLED, AGENT FROM AR_DN ";
                    lSQL = "SELECT DOCNO,CODE,DOCDATE,DOCAMT,LOCALDOCAMT,CURRENCYCODE,CURRENCYRATE,PAYMENTAMT,CANCELLED, AGENT FROM AR_DN ";
                    if (cms_updated_time.Count > 0)
                    {
                    ...
                    try
                    {
                        ...
                        //query = "INSERT INTO cms_debitnote(dn_code, cust_code, dn_date, dn_amount, outstanding_amount, cancelled, salesperson_id) VALUES ";
                        //updateQuery = " ON DUPLICATE KEY UPDATE cancelled = VALUES(cancelled), dn_amount = VALUES(dn_amount), outstanding_amount = VALUES(outstanding_amount), cust_code = VALUES(cust_code), salesperson_id = VALUES(salesperson_id);";
                        query = "INSERT INTO cms_debitnote(dn_code, cust_code, dn_date, dn_amount, outstanding_amount, cancelled) VALUES ";
                        ...
                        while (!lDataSet.eof)
                        {
                            ...
                            double outstandingAmt = docAmt - paymentAmt; //just added 08092020; DN & INV 30092020
                            ...
                            string Values = string.Format("('{0}','{1}','{2}','{3}','{4}','{5}')", DOCNO, CODE, DOCDATE, DOCAMT, OUTSTANDINGAMT, CANCELLED);
                        ...
                        if (inDBdn.Count > 0)
                        {
                            logger.Broadcast("Total debit note records to be deactivated: " + inDBdn.Count);

                            string inactive = "UPDATE cms_debitnote SET cancelled = '{0}' WHERE dn_code = '{1}'";
                            ...
                            logger.Broadcast(inDBdn.Count + " debit note records deactivated");

                            inDBdn.Clear();
                        }

                        _mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES ('cms_debitnote', NOW()) ON DUPLICATE KEY UPDATE table_name = VALUES(table_name), updated_at = VALUES(updated_at)");
