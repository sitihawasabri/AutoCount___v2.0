...
{
    public class JobATCItemTemplateDtlSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCItemTemplateDtlSync;                               
                    slog.action_details = Constants.Tbl_cms_package_dtl + Constants.Is_Starting;                   
                    ...
                    logger.message = "ATC item template details sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        {
                            throw new Exception("ATC item template details sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            string insertQuery = "INSERT INTO cms_package_dtl (@columns) VALUES @values ON DUPLICATE KEY UPDATE @update_columns";
                    ...
                    LocalDB.InsertSyncLog(slog);
                    logger.message = "ATC item template details sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCItemTemplateDtlSync",
                    exception = e.Message,
