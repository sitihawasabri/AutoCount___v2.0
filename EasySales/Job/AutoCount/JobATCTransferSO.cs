            ...
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        string SOQuery = string.Empty;                              //order_id = "\"SO-SS-001\",\"SO-SS-003\",\"SO-SS-002\"";
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < queryResult.Count; i++)
                            {
                                ...
                                if (branchCode == "NULL")
                                {
                                    logger.Broadcast("branchCode == NULL");
                                ...
                                for (int idx = 0; idx < allOrderItems.Count; idx++)
                                {
                                    ...
                                    logger.Broadcast("subTotal ===> " + subTotal);
                                    ...
                                    logger.Broadcast("product code ===> " + product_code);
                                    ...
                                    logger.Broadcast("Tax.Count ===> " + Tax.Count);
                                    ...
                                    logger.Broadcast("_maxDelKey ===> " + _maxDelKey);

                                    item["unit_uom"] = item["unit_uom"].Replace("'", "''");
                                    logger.Broadcast("unit_uom ===> " + item["unit_uom"]);
                                    ...
                                    if (disc1 > 0 || disc2 > 0 || disc3 > 0)
                                    {
                                        ...
                                        logger.Broadcast("disc_1 ===> " + item["disc_1"]);
                                        logger.Broadcast("disc_2 ===> " + item["disc_2"]);
                                        logger.Broadcast("disc_3 ===> " + item["disc_3"]);
                                        subTotal = calcDiscount(unitPrice, quantity, disc1, disc2, disc3);
                                        logger.Broadcast("subTotal ===> " + subTotal);
                                    }

                                    logger.Broadcast("disc1 ===> " + disc1);
                                    logger.Broadcast("disc2 ===> " + disc2);
                                    logger.Broadcast("disc3 ===> " + disc3);
                                    logger.Broadcast("subTotal ===> " + subTotal);
                                    ...
                                    logger.Broadcast("stmtu ===> " + stmtu);
                                    ArrayList itemUom = mssql.Select(stmtu);
                                    logger.Broadcast("itemUom ===> " + itemUom.Count);
                                    ...
                                    if (itemUom.Count > 0)
                                    {
                                        ...
                                        logger.Broadcast("u_rate ===> " + u_rate); 
                                        logger.Broadcast("weight ===> " + weight); 
                                        logger.Broadcast("volume ===> " + volume);

                                        uomList.Clear();
                                    }
                                    else
                                    {
                                        logger.Broadcast("proceedNextItems");
                                        string updateItemFault = "UPDATE cms_order_item SET item_fault_msg = 'Item does not exists in AutoCount' WHERE order_item_id = '" + order_item_id + "'";
                                    ...
                                    logger.Broadcast("furtherDescription ===> " + furtherDescription);
                                    logger.Broadcast("atc_description ===> " + atc_description);
                                ...
                                if (fault == 0 && ValuesDtlList.Count > 0)
                                {
                                    ...
                                    mssql.Message("insertSO Query ====> " + insertSO);
                                    if (!insertedOrder)
                                    {
                                    ...
                                    mssql.Message("insertedPicked ========>" + insertedPicked);
                                    if (!insertedDtl)
                                    {
                                    ...
                                    if (fault == 0)
                                    {
                                        ...
                                        //string updateOrderStatusQuery = "UPDATE cms_order SET order_status = '" + updateOrderStatus + "', order_remark = CONCAT('[DIRECT DB] Transferred from EXE at ', NOW()) WHERE order_id = '" + orderId + "'";
                                        string updateOrderStatusQuery = "UPDATE cms_order SET order_status = '" + updateOrderStatus + "' WHERE order_id = '" + orderId + "'";
                                        mysql.Insert(updateOrderStatusQuery);
                                        mysql.Message("UPDATE ORDER STATUS ATC -----> " + updateOrderStatusQuery);
...
//                                    string insertedPicked = ss_sql_dtl + string.Join(", ", ValuesDtlList);
//                                    bool insertedDtl = mssql.Insert(insertedPicked);
//                                    mssql.Message("insertedPicked Query ====> " + insertedPicked);
//                                    if (!insertedDtl)
//                                    {
//                                        mssql.Message("[" + insertedPicked + "] Order dtl did not transfer due to false return by MSSQL");
//                                        fault++;
...
//                                        mysql.Insert(updateOrderStatusQuery);
//                                        mysql.Message("updateOrderStatusQuery =====>" + updateOrderStatusQuery);
//                                        logger.Broadcast(orderId + " created");
//                                    }
//                                }
//                                ValuesDtlList.Clear();
