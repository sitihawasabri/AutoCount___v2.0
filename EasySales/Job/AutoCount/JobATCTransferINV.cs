...
namespace EasySales.Job
{
    ...
    public class JobATCTransferINV : IJob
    {
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    logger.message = "ATC Transfer INV is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        {
                            throw new Exception("ATC Transfer INV sync requires backend rules");
                        }
                        ...
                        if (socket_OrderId != string.Empty)
                        {
                            ...
                            SOQuery = "SELECT cms_order.*, cms_login.staff_code FROM cms_order LEFT JOIN cms_login ON cms_order.salesperson_id = cms_login.login_id WHERE order_status = " + order_status + " AND cancel_status = 0 AND doc_type = 'invoice' AND order_fault = 0 AND order_id IN (" + socket_OrderId + ")";
                            mysql.Message("with order id: " + SOQuery);
                            logger.Broadcast("with order id: " + SOQuery);
                        }
                        else
                        {
                            SOQuery = "SELECT cms_order.*, cms_login.staff_code FROM cms_order LEFT JOIN cms_login ON cms_order.salesperson_id = cms_login.login_id WHERE order_status = " + order_status + " AND cancel_status = 0 AND doc_type = 'invoice' AND order_fault = 0";
                            mysql.Message("W/O order id: " + SOQuery);
                            logger.Broadcast("W/O order id: " + SOQuery);
                        }
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < queryResult.Count; i++)
                            {
                                ...
                                ArrayList dockeys = mssql.Select("(SELECT ISNULL(MAX(DocKey)+1,0) as dockey FROM dbo.IV WITH(SERIALIZABLE, UPDLOCK))"); //{[dockey, 61239]}
                                ...
                                string stmt_maxDelKey = "(SELECT ISNULL(MAX(DtlKey)+1,0) as maxDtlkey FROM dbo.IVDTL WITH(SERIALIZABLE, UPDLOCK))";
                                ...
                                if (fault == 0)
                                {
                                    ...
                                    logger.Broadcast(insertSO);

                                    mssql.Message(insertSO);
                                    if (!insertedOrder)
                                    {
                                        logger.Broadcast("[" + orderId + "] Order did not transfer due to false return by MSSQL");
                                    ...
                                    string ss_sql_dtl = "INSERT INTO dbo.IVDTL (DtlKey,DocKey,Seq,MainItem,ItemCode,FurtherDescription,Location,Description,UOM,UserUOM,Qty,Rate,SmallestQty,TransferedQty,FOCQty,SmallestUnitPrice,UnitPrice,DiscountAmt,Tax,SubTotal,LocalSubTotal,Transferable,PrintOut,DtlType,AddToSubTotal,SubTotalExTax,LocalTax,DeliveryDate,TaxableAmt,LocalSubTotalExTax,TaxRate,LocalTaxableAmt,TaxCurrencyTax,TaxCurrencyTaxableAmt,Discount,AccNo) VALUES "; //StockReceived --- REMOVED FOR CHILLMI
                                    string insertedPicked = ss_sql_dtl + string.Join(", ", ValuesDtlList);
                                    mysql.Message("insertedPicked: " + insertedPicked);
                                    bool insertedDtl = mssql.Insert(insertedPicked);
                                    logger.Broadcast("Inserting order items");
                                    mssql.Message(insertedPicked);
                                    logger.Broadcast(insertedPicked);
                                    if (!insertedDtl)
                                    {
                                        logger.Broadcast("[" + insertedPicked + "] Order dtl did not transfer due to false return by MSSQL");
