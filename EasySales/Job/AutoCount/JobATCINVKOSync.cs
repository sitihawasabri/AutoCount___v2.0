        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCINVKO;
                    ...
                    logger.message = "ATC Aging KO Invoice sync is running";
                    logger.Broadcast();
                       ...
                       // Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_customer_ageing_ko_inv_atc");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT inv.DocNo AS doc_ko_ref,pm.DocNo AS doc_code,'INV' AS doc_ko_type,ko.Amount AS doc_amount,'1' AS active_status, 'OR' as doc_type FROM ARPayment AS pm LEFT JOIN ARPaymentKnockOff ko ON ko.DocKey = pm.DocKey LEFT JOIN ARInvoice inv ON inv.DocKey = ko.KnockOffDocKey WHERE ko.KnockOffDocType = 'RI'; ";
                        ...
                        {
                            throw new Exception("ATC Aging KO Invoice sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    logger.message = string.Format("{0} Aging KO INV records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                    ...
                    logger.message = "ATC Aging KO Invoice sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
