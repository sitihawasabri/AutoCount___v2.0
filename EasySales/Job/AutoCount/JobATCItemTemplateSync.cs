...
{
    public class JobATCItemTemplateSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCItemTemplateSync;                               
                    slog.action_details = Constants.Tbl_cms_package + Constants.Is_Starting;                   
                    ...
                    logger.message = "ATC item template sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_package_atc");

                        //Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_package");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT PackageCode, Description, UnitPrice, IsActive, UserUOM FROM dbo.Package";
                        ...
                        {
                            throw new Exception("ATC item template sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("Item Template to be inserted: " + queryResult.Count);
                            }

                            string mysql_insert = string.Empty;
                            string mssql_insert = string.Empty;

                            string insertQuery = "INSERT INTO cms_package (@columns) VALUES @values ON DUPLICATE KEY UPDATE @update_columns";
                            ...
                            //$db2->query("UPDATE cms_package_dtl SET dtl_status = 0 WHERE dtl_parent = '{$row->PackageCode}'");
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                database.Include.Iterate<Dictionary<string, string>>((include, inIdx) =>
                                {
                                    ...
                                    {
                                        string pkg_status = string.Empty;
                                        ...
                                        row += inIdx == 0 ? "('" + pkg_status + "" : "','" + pkg_status;
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    logger.message = string.Format("{0} item template records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                            });
                            ...
                            mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_package', NOW()) ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)");
                    ...
                    LocalDB.InsertSyncLog(slog);
                    logger.message = "ATC item template sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCItemTemplateSync",
                    exception = e.Message,
