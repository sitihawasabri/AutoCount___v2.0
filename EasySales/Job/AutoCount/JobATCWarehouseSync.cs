...
{
    public class JobATCWarehouseSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCWarehouseSync;                                 /*check again */
                    slog.action_details = Constants.Tbl_cms_warehouse + Constants.Is_Starting;                   /*check again */
                    ...
                    logger.message = "ATC warehouse sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_warehouse_atc");

                       // Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_warehouse");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT Location as wh_code ,Description as wh_name ,Desc2 as wh_remark, IsActive as status FROM Location";
                        ...
                        {
                            throw new Exception("ATC Warehouse sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("Warehouse to be inserted: " + queryResult.Count);
                            }

                            string mysql_insert = string.Empty;
                            string mssql_insert = string.Empty;

                            string insertQuery = "INSERT INTO cms_warehouse (@columns) VALUES @values ON DUPLICATE KEY UPDATE @update_columns";
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                database.Include.Iterate<Dictionary<string, string>>((include, inIdx) =>
                                {
                                    ...
                                    if (find_mssql_field == "status")
                                    {
                                        string warehouseStatus = string.Empty;

                                        map.Iterate<KeyValuePair<string, string>>((mssql_fields, mssqIndx) =>
                                        {
                                            if (mssql_fields.Key == "status")
                                            {
                                                RecordCount++;
                                                warehouseStatus = mssql_fields.Value;
                                                warehouseStatus.ToLower();

                                                if (warehouseStatus == "T")
                                                {
                                                    warehouseStatus = "1";
                                                }
                                                else
                                        ...
                                        row += inIdx == 0 ? "('" + warehouseStatus + "" : "','" + warehouseStatus;
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    logger.message = string.Format("{0} warehouse records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                            });
                            ...
                            mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_warehouse', NOW()) ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)");
                    ...
                    slog.action_identifier = Constants.Action_ATCWarehouseSync;
                    ...
                    LocalDB.InsertSyncLog(slog);
                    logger.message = "ATC warehouse sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
