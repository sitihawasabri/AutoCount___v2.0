...
namespace EasySales.Job
{
    ...
    public class JobATCStockCategorySync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCStockCategorySync;
                    ...
                    logger.message = "ATC stock category sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_product_category_atc");

                        //Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_product_category");
                            ...
                            foreach (var key in jsonRule)
                            {
                                if (key.mssql != null && key.mssql.GetType().ToString() == "Newtonsoft.Json.Linq.JArray")
                                {
                                    ...
                                    foreach (var db in mssql_rules)
                                    {
                                        ...
                                        string categoryField = db.category_field;

                                        if(categoryField == "ItemType") //ItemType
                                        {
                                            query = "SELECT * FROM dbo.ItemType";
                                        }
                                        else //itemGroup
                                        {
                        ...
                        {
                            throw new Exception("ATC Product Category sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("Product Category to be inserted: " + queryResult.Count);
                            }

                            mssql.Message("[JobATCStockCategorySync] queryResult.Count ----> " + queryResult.Count);
                            string mysql_insert = string.Empty;
                            string mssql_insert = string.Empty;

                            string insertQuery = "INSERT INTO cms_product_category (@columns) VALUES @values ON DUPLICATE KEY UPDATE @update_columns";
                            ...
                            int moderator = 0;
                            ArrayList checkColumnExist = mysql.Select("SHOW COLUMNS FROM cms_product_category LIKE 'moderator';");
                            ...
                            if (moderator == 1)
                            {
                                insertQuery = insertQuery.Replace("category_status=VALUES(category_status)", "category_status=IF(moderator = '' OR moderator IS NULL, VALUES(category_status), category_status)");
                            }
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    mysql.Message("ATC Category query ---> " + insertQuery);

                                    insertQuery = insertQuery.ReplaceAll("@values", values);
                                    valueString.Clear();

                                    logger.message = string.Format("{0} stock category records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                            });
                            ...
                            mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_product_category', NOW()) ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)");
                    ...
                    logger.message = "ATC stock category sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCStockCategorySync",
                    exception = e.Message,
