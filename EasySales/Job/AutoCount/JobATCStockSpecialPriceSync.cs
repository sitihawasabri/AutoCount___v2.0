...
namespace EasySales.Job
{
    ...
    public class JobATCStockSpecialPriceSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCStockSpecialPriceSync;                                 
                    ...
                    logger.message = "ATC stock special price sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_product_price_v2_atc");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT ItemCode, PriceCategory, AccNo, FixedPrice, UOM FROM dbo.ItemPrice";
                        ...
                        {
                            throw new Exception("ATC Stock Special Price sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("Product Special Price to be inserted: " + queryResult.Count);
                            }
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    mysql.Message("Stock Special Price ====> " + insertQuery);
                            ...
                            mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_product_price_v2', NOW()) ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)");
                    ...
                    logger.message = "ATC stock special price sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCStockSpecialPriceSync",
                    exception = e.Message,
