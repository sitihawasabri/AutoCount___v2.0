...
namespace EasySales.Job
{
    ...
    class JobATCInvoiceDetailSync :IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCInvoiceDetailSync;
                    ...
                    logger.message = "ATC invoice detail sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT IVDTL.DocKey, IVDTL.ItemCode, IVDTL.UnitPrice , IVDTL.Qty , IVDTL.UOM, IVDTL.Description, IVDTL.Discount, IVDTL.SubTotal, IV.DocNo FROM dbo.IVDTL INNER JOIN dbo.IV on IV.DocKey = IVDTL.DocKey WHERE ItemCode IS NOT NULL";
                        ...
                        {
                            throw new Exception("ATC Invoice Details sync requires backend rules");
                        }
                    ...
                    logger.message = "ATC invoice detail sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCInvoiceDetailSync",
                    exception = e.Message,
