        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCCNKO;
                    ...
                    logger.message = "ATC Aging KO Credit Note sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_customer_ageing_ko_cn_atc");

                        //Dictionary<string, string> cms_updated_time = mysql.GetUpdatedTime("cms_customer_ageing_ko_cn_atc");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT cn.DocNo AS doc_code,'INV' AS doc_ko_type, ko.Amount AS doc_amount, inv.DocNo AS doc_ko_ref,'1' AS active_status, 'CN' as doc_type FROM ARCNKnockOff ko LEFT JOIN ARCN cn ON cn.DocKey = ko.DocKey LEFT JOIN ARInvoice inv ON inv.DocKey = ko.KnockOffDocKey WHERE ko.KnockOffDocType = 'RI'; ";
                        ...
                        {
                            throw new Exception("ATC Aging KO Credit Note sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("CN-KO to be inserted: " + queryResult.Count);
                            }
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                if (valueString.Count % 2000 == 0)
                                {
                                    ...
                                    logger.message = string.Format("{0} Aging KO Credit Note records is inserted into " + mysqlconfig.config_database, RecordCount);
                                    logger.Broadcast();
                                }
                    ...
                    logger.message = "ATC Aging KO Credit Note sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
