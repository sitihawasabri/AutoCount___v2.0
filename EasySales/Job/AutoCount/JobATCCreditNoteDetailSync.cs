...
namespace EasySales.Job
{
    ...
    class JobATCCreditNoteDetailSync : IJob
    {
        public void Execute()
        {
            this.Run();
        ...
        public void Run()
        {
            try
            {
                Thread thread = new Thread(p =>
                {
                    ...
                    slog.action_identifier = Constants.Action_ATCCreditNoteDetailSync;
                    ...
                    logger.message = "ATC credit note details sync is running";
                    logger.Broadcast();
                    ...
                    mysql_list.Iterate<DpprMySQLconfig>((mysql_config, index) =>
                    {
                        ...
                        dynamic jsonRule = checkDB.CheckTablesExist().GetSettingByTableName("cms_creditnote_details_atc");
                        ...
                        if (jsonRule != null)
                        {
                            foreach (var key in jsonRule)
                            {
                                ...
                                {
                                    string query = "SELECT CNDTL.DocKey, CNDTL.ItemCode, CNDTL.UnitPrice , CNDTL.Qty , CNDTL.UOM, CNDTL.Description, CNDTL.Discount, CN.DocNo FROM dbo.CNDTL INNER JOIN dbo.CN on CN.DocKey = CNDTL.DocKey WHERE ItemCode IS NOT NULL";
                        ...
                        {
                            throw new Exception("ATC Credit Note Details sync requires backend rules");
                        }
                        ...
                        mssql_rule.Iterate<ATCRule>((database, idx) =>
                        {
                            ...
                            {
                                logger.Broadcast("CN Details to be inserted: " + queryResult.Count);
                            }
                            ...
                            queryResult.Iterate<Dictionary<string, string>>((map, i) =>
                            {
                                ...
                                database.Include.Iterate<Dictionary<string, string>>((include, inIdx) =>
                                {
                                    ...
                                    if (find_mssql_field == "total")
                                    {
                                        ...
                                        {
                                            if (mssql_fields.Key == "UnitPrice")
                                            {
                                                unitPrice = mssql_fields.Value;
                                            }

                                            if (mssql_fields.Key == "Qty")
                                            {
                                        ...
                                        double.TryParse(qty, out double Qty);
                                        double total = UnitPrice * Qty;

                                        row += inIdx == 0 ? "('" + total + "" : "','" + total;
                            ...
                            mysql.Insert("INSERT INTO cms_update_time(table_name, updated_at) VALUES('cms_creditnote_details', NOW()) ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)");
                    ...
                    logger.message = "ATC credit note details sync finished in (" + ts.TotalSeconds.ToString("F") + " seconds)";
                    logger.Broadcast();
            ...
            catch (ThreadAbortException e)
            {
                ...
                {
                    file_name = "JobATCCreditNoteDetailSync",
                    exception = e.Message,
